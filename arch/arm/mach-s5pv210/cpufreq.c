...
static unsigned int g_dvfs_high_lock_limit = 5; //1Ghz
			  ...
			  unsigned int relation)
{
	...
	if ((index == L0) || (priv_index == L0))        // 1.2Ghz
		pll_changing = 1;
	else if ((index == L1) || (priv_index == L1))   // 1.1Ghz
		pll_changing = 1;
...
static int __init s5pv210_cpu_init(struct cpufreq_policy *policy)
{
	...
	policy->cpuinfo.transition_latency = 30000; // 40000
